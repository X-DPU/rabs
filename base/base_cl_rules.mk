...
APP_SRC = $(wildcard $(UPPER_DIR)/$(APP_DIR)/*.cl)
APP_SRC_RMDIR = $(patsubst $(UPPER_DIR)/$(APP_DIR)/%, %,$(APP_SRC))
APP_OBJS = $(patsubst %.cl, %,$(APP_SRC_RMDIR))
...
SET_KERNEL_NAME = $(eval KERNEL_NAME=$(patsubst $(dir $<)%.cl,%, $<))
SET_LOCAL_CONFIG_FILE = $(eval LOCAL_CONFIG_FILE=$(patsubst %.cl,%.ini, $<))



$(TEMP_DIR)/$(UPPER_DIR)/$(APP_DIR)/%.xo: $(UPPER_DIR)/$(APP_DIR)/%.cl
	$(SET_APP_DIR)
	$(SET_KERNEL_NAME)
